...
namespace GatewayAPI.Services
{
    public class CartRepo : IRepo<int, ShoppingCartItemDTO>
    {
        ...
        public async Task<ShoppingCartItemDTO> GetT(ShoppingCartItemDTO item)
        {
            using (_httpClient)
            {
                StringContent content = new StringContent(JsonConvert.SerializeObject(item), Encoding.UTF8, "application/json");
                using (var response = await _httpClient.PostAsync("http://localhost:5041/api/Cart/CheckForCartItem", content))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        string responseText = await response.Content.ReadAsStringAsync();
