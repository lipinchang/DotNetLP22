...
namespace GatewayAPI.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        [Route("GetAllCarts")]
        [HttpGet]
        public async Task<ActionResult<IEnumerable<ShoppingCartItemDTO>>> IndexAsync()
        {
            var items = await _repo.GetAll();
            return Ok(items.ToList());
        }

        [Route("CheckForCartItem")]  //in use
        [HttpPost]
        public async Task<ActionResult<ShoppingCartItemDTO>> Get(ShoppingCartItemDTO item)
        {
            var itemInCart = await _repo.GetT(item);
            //if(itemInCart == null)
            //    return NotFound();
